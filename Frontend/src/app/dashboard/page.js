...
import { ShoppingBag, Zap, ArrowLeft, Sparkles, Loader2, Copy, Check } from "lucide-react";
        ...
        {lastFinalOrder ? (
          <div className="flex flex-col gap-3">
            <div className="bg-[#f8f9fa] border-[2px] border-black p-4 rounded-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
              <div>
                {lastFinalOrder.items.map((item, i) => (
              ...
              <span className={`text-[10px] font-black px-3 py-1 border-[2px] rounded-lg shrink-0 ${STATUS_COLOR[lastFinalOrder.status] || "bg-gray-100 border-black"}`}>
                {STATUS_LABEL[lastFinalOrder.status] || lastFinalOrder.status}
              </span>
            </div>

            {lastFinalOrder.items.some((i) => i.credentials) && (
              <div className="flex flex-col gap-2">
                {lastFinalOrder.items.filter((i) => i.credentials).map((item, i) => (
