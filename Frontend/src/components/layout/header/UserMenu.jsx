...
export default function UserMenu() {
  const { isLoggedIn, userName, logout } = useAuthStatus();
  const [isOpen, setIsOpen] = useState(false);

  return (
    <div className="relative flex items-stretch">
      {isLoggedIn ? (
        /* ================= حالت لاگین‌شده: نمایش نام کاربر ================= */
        <div
        ...
        >
          <button className="flex items-center gap-2 px-5 bg-[#12e2a3] hover:bg-[#0fd196] font-black text-sm md:text-base text-black transition-colors cursor-pointer">
            ...
            <span>{userName}</span>
            <ChevronDown
              className={`w-4 h-4 stroke-[3] transition-transform ${isOpen ? "rotate-180" : ""}`}
            />
          </button>
        ...
        /* ================= حالت مهمان: دکمه ورود و ثبت‌نام ================= */
        <Link
          href="/auth"
