...
export default function Filters({
  ...
  return (
    <aside
      className={`w-full bg-white border-[3px] border-black rounded-[16px] ${
        mobile ? "p-3 gap-3" : "p-5 gap-6 sticky top-20"
      } shadow-[-6px_6px_0_0_rgba(0,0,0,1)] dir-rtl flex flex-col z-10 transition-all duration-300`}
    >
      {/* هدر فیلترها */}
      ...
      <div ref={sortRef} className="relative">
        ...
        >
          <span>{SORT_OPTIONS.find((o) => o.value === sortBy)?.label || "انتخاب کنید"}</span>
          <ChevronDown className={`w-4 h-4 stroke-[3] transition-transform ${sortOpen ? "rotate-180" : ""}`} />
        </button>
        {sortOpen && (
          <div className="absolute top-full right-0 left-0 mt-1 bg-white border-[2.5px] border-black rounded-xl shadow-[-4px_4px_0_0_rgba(0,0,0,1)] z-50 overflow-hidden">
            {SORT_OPTIONS.map((opt) => (
              <button
                key={opt.value}
                onClick={() => { setSortBy(opt.value); setSortOpen(false); }}
                className={`w-full text-right px-4 py-2 font-black text-sm hover:bg-gray-100 ${sortBy === opt.value ? "bg-[#ccff00]" : ""}`}
              >
                {opt.label}
              </button>
            ))}
      ...
      <div>
        ...
        <div className="flex items-center gap-2 mt-3">
          <div className="flex-1">
            ...
            <input
              type="number"
              min={MIN} max={priceRange?.max ?? MAX} step={STEP}
              value={priceRange?.min ?? MIN}
          ...
          <div className="flex-1">
            ...
            <input
              type="number"
              min={priceRange?.min ?? MIN} max={MAX} step={STEP}
              value={priceRange?.max ?? MAX}
