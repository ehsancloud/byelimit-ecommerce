...
export default function MobileFilterDrawer({ isOpen, setIsOpen, filterProps }) {
  ...
  return (
    <>
      {/* دکمه شناور دایره‌ای در پایین سمت چپ */}
      <button
      ...
      <div
        className={`md:hidden fixed bottom-0 left-0 right-0 z-50 bg-[#f3f3f3] border-t-[4px] border-black rounded-t-[24px] p-5 transition-transform duration-300 h-auto max-h-full overflow-hidden ${
...
export default function MobileFilterDrawer({ isOpen, setIsOpen, filterProps }) {
  return (
    <>
      {/* دکمه شناور دایره‌ای در پایین سمت راست */}
      <button
        onClick={() => setIsOpen(true)}
        className="md:hidden fixed bottom-6 right-6 z-40 w-14 h-14 bg-[#ccff00] border-[3px] border-black rounded-full flex items-center justify-center shadow-[-4px_4px_0px_0px_rgba(0,0,0,1)] active:translate-x-[-2px] active:translate-y-[2px] active:shadow-none transition-all"
        aria-label="باز کردن فیلترها"
      >
        <SlidersHorizontal className="w-7 h-7 stroke-[2.5] text-black" />
      </button>
      ...
      {/* منوی بازشونده از پایین */}
      <div
        className={`md:hidden fixed bottom-0 left-0 right-0 z-50 bg-[#f3f3f3] border-t-[4px] border-black rounded-t-[24px] p-5 transition-transform duration-300 max-h-[85vh] overflow-y-auto ${
          isOpen ? "translate-y-0" : "translate-y-full"
        }`}
      >
        <div className="flex justify-between items-center mb-4 border-b-[2px] border-black pb-3">
